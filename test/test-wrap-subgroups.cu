...
BOOST_DATA_TEST_CASE(test_wrap_subgroups, test_data, td) {
	...
	managed_vector<int> subgroup_ptrs(td.subgroup_ptrs.begin(),
	                                  td.subgroup_ptrs.end());
	managed_vector<int> group_ptrs(
	    std::max(num_elements / mean_group_size + 1, 2));

	managed_vector<int> group_ptrs_size_(1);
	auto group_ptrs_size =
	    gsl_lite::make_not_null<int*>(group_ptrs_size_.data());

	cudaStream_t stream = 0;

	auto memory_resource = pmr::delayed_pool_type<pmr::managed_resource_type>();

	async::wrap_subgroups<int>(stream,
	                           subgroup_ptrs,
	                           num_elements,
	                           group_ptrs,
	                           mean_group_size,
